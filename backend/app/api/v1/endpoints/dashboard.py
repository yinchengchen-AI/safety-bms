...
def get_stats(
    ...
    # 服务工单状态分布
    service_query = db.query(ServiceOrder.status, func.count(ServiceOrder.id)).group_by(
        ServiceOrder.status
    )
    ...
    contract_amount_by_service_query = apply_data_scope(
        contract_amount_by_service_query, Contract, current_user
    )
    contract_amount_by_service_results = contract_amount_by_service_query.all()
    contract_amount_by_service_type = [
        {
            "service_type": enum_value(r.service_type),
    ...
    # 4. 本月新增服务工单数
    monthly_new_service_orders_query = db.query(func.count(ServiceOrder.id)).filter(
        func.extract("year", ServiceOrder.created_at) == current_year,
        func.extract("month", ServiceOrder.created_at) == current_month,
    )
    monthly_new_service_orders_query = apply_data_scope(
        monthly_new_service_orders_query, ServiceOrder, current_user
    )
    monthly_new_service_orders = monthly_new_service_orders_query.scalar() or 0

    return {
        "contract_status_distribution": [
            {"status": enum_value(s), "count": c} for s, c in contract_status_dist
        ],
        "service_status_distribution": [
            {"status": enum_value(s), "count": c} for s, c in service_status_dist
        ],
