...
from app.schemas.service import (
    ServiceOrderCreate, ServiceOrderUpdate, ServiceOrderOut, ServiceOrderListOut,
    ServiceOrderStatusUpdate, ServiceReportOut,
)
...
def _get_service_order_with_relations(db: Session, order_id: int) -> Optional[ServiceOrder]:
    return (
...
def export_service_orders(
    ...
    headers = ["工单编号", "标题", "客户名称", "服务类型", "负责人", "状态", "计划开始", "计划结束", "创建时间"]
    rows = []
    for item in items:
        rows.append([
    ...
    return export_excel_response(f"service_orders_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx", headers, rows)
...
def get_service_order(
    ...
    result.customer_name = order.contract.customer.name if order.contract and order.contract.customer else None
...
def update_service_order(
    ...
    result.customer_name = updated.contract.customer.name if updated.contract and updated.contract.customer else None
...
def update_status(
    ...
    if updated.assignee_id and updated.assignee_id != current_user.id:
        if body.status == ServiceOrderStatus.IN_PROGRESS and old_status != ServiceOrderStatus.IN_PROGRESS:
            notification_service.create(
                db,
        ...
        elif body.status == ServiceOrderStatus.COMPLETED and old_status != ServiceOrderStatus.COMPLETED:
            notification_service.create(
                db,
                user_id=updated.assignee_id,
                title="服务工单已完成",
